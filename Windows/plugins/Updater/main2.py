...
try:
	...
	print(Fore.RED + '[!] ' + Fore.YELLOW + 'Failed to connect the server. Check your internet Connection.')
	End()
if Size == '!502!':
...
Choose = None
while True:
	Choose = raw_input(Fore.YELLOW + '[?] ' + Fore.WHITE + 'Need to download ' + Size + ', Confirm and update [y/n] ?  ')
	if str.lower(Choose) == 'y' or str.lower(Choose) == 'yes':
		...
		for root, dirs, files in os.walk(Location):
			for file in files:
				if file.endswith(".py"):
					os.chmod(os.path.join(root, file),771)
