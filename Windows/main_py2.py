...
init()
try:
	execfile(sys.path[0] + '/lib/functionsPyTwo.py')
except:
...
def Generate_Menu(Plugin_List):
	...
	for Plugin in Plugin_List:
		ret += Fore. + str(Plugin[0]),'green') + ' ' + Fore. + Plugin[2],'white')
	return ret
try:
...
for Name in List:
	...
	if not (os.path.isfile(Location + '/plugins/' + Name + '/main2.py') or os.path.isfile(Location + '/plugins/' + Name + '/main.py')):
		if os.path.isfile(Location + '/plugins/' + Name + '/main3.py'):
			...
			print(Fore.YELLOW + "Becase it's writen for Python3 and you're using Python2.")
		else:
	...
	if os.path.isfile(Location + '/plugins/' + Name + '/main2.py'):
		try:
			...
			print(Fore.RED + '[!] Failed to load "' + Name + '" plugin.')
			print(Fore.RED + 'Becase "main2.py" file in script folder not have read permission.')
			try:
				File.close()
			except:
				pass
...
while True:
	...
	try:
		Choose = int(raw_input(Fore.BLUE + '\n\nEnter your number: ' + Fore.WHITE))
	except:
		try:
	...
	if Find:
		try:
			Choose = raw_input(Fore.YELLOW + '[?] ' + Fore.WHITE + ' Work finished. Do you want exit from script ? [y/n] ')
		except:
			Clear()
			print(Fore.MAGENTA + 'Good by..')
			exit(0)
